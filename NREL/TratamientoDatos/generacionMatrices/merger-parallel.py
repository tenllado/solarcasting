...
# Stations to consider: remove ap3 because it is very faulty
...
def processFile(input_file,input_folder,output_folder):
	valid = True
	...
	for station in stations:
		...
		#    DEC = util_csm.declination_degree(datetime,TY)
			...
			if simple_csm == 'mcclear':
				ghi_csm = mcclear_model.get_irradiance(datetime)
				#print("Mcclear. date: " , datetime , "  csk: " , ghi_csm) 
...
if __name__ == '__main__':
	...
	# if using mcclear model, load the data file.
	## NOTE THAT THIS IS HARDCODED for Hawai (and always using the model for dh6 location)
	if simple_csm == 'mcclear':
		mcclear_model = mc.McClear('mcclear-dhhl6-2010-2011.csv')
		print("McClear model loaded. File dhhl6-2010-2011.csv")

	input_folder = cfg_data['orig_folder']
	output_folder = cfg_data['dest_folder'] + '/' + simple_csm
	...
	#results = Parallel(n_jobs=num_cores)(delayed(processFile)(input_file,input_folder,output_folder) for input_file in input_files)
	#ninvalid = nfiles - sum(results)

	#using imap
	with mp.Pool(num_cores) as p:
		args = [ (input_file,input_folder,output_folder) for input_file in input_files ]
		#print("args ", args)
		#print(p.map(processFile_unpack, args))
		print(p.starmap(processFile, args))

	#ninvalid = nfiles - sum(results)


	#for input_file in input_files:
	#	valid = processFile(input_file,input_folder,output_folder)
	#	if (valid == False):
	#		ninvalid+=1
